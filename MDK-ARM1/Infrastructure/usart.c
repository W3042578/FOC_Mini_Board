...
#define   USART_OSCILL0_BUFFER    4096					//8位数组缓冲区大小
#define   SEND_NUMBER             32						//发包组帧数  后续考虑固定回发数据长度，不变化回发包总长度
//全局定义
      ...
      if(interval == 0)
      {
        switch (Test_Uart)//获取数据
        {
          case 0:
            ...
            break;
          case 1:
            Debug_Buffer[0].flo = (float)Motor1.Ia;
            Debug_Buffer[1].flo = (float)Motor1.Ib;
            size = 2;
            interval = 2;
            break;
          case 2:
            ...
            break;
          case 4:
//            Debug_Buffer[0].flo = (float)Motor1.Mechanical_Angle;Encoder1.Encoder_Pulse
						Debug_Buffer[0].flo = Test1;
            size = 1;
            interval = 1;
          break;
          default:
        ...
        (buffer_turn_differ == 1 && (write_address + frame_length) < read_address)) && temp)
        {
        ...
        //读出等待触发，等待再次采集到数据后继续发送        
        if(_TEST(&uart_status_byte, USART_WAIT_SEND))     
				{
					if(((buffer_turn_differ == 0) && (read_address + (frame_length * SEND_NUMBER) <= write_address)) || \
					((buffer_turn_differ == 1) && ((frame_length * SEND_NUMBER <= write_address) || \
					(read_address + frame_length * SEND_NUMBER <= USART_OSCILL0_BUFFER))))		
					{
  ...
  {    
    if(buffer_turn_differ == 0 && (read_address + (frame_length * SEND_NUMBER) > write_address))
    {
      ...
      _SET(&uart_status_byte, USART_WAIT_SEND);
		...
		//继续读取条件：
		//1.写入与读出同环，写入超前读出
		//2.写入超前读出一环，写入地址大于0起始指定帧数数据大小 || 读出地址加上后不回环
    else if(((buffer_turn_differ == 0) && (read_address + (frame_length * SEND_NUMBER) <= write_address)) || \
    ((buffer_turn_differ == 1) && ((frame_length * SEND_NUMBER <= write_address) || \
		(read_address + frame_length * SEND_NUMBER <= USART_OSCILL0_BUFFER))))
    {
      oscilloscope_send();
